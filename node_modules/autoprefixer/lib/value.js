(function() {
  ...
  Value = (function(superClass) {
    ...
    Value.save = function(prefixes, decl) {
      var already, cloned, isTransition, prefix, prefixed, prop, propPrefix, ref, results, rule, trimmed, value;
      prop = decl.prop;
      ...
      for (prefix in ref) {
        ...
        isTransition = prop === 'transition' || prop === 'transition-property';
        propPrefix = vendor.prefix(prop);
        if (propPrefix === prefix) {
          results.push(decl.value = value);
        } else if (isTransition && value.indexOf('-webkit-filter') !== -1) {
          results.push(decl.value = value);
